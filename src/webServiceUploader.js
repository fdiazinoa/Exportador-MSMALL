...
class WebServiceUploader {
  ...
  _buildSyncPayload({ wsConfig, job, mappedData, filePath }) {
    const fileName = filePath ? path.basename(filePath) : undefined;
    return {
      mall_id: wsConfig.mallId,
      local_id: wsConfig.localId,
      rows: mappedData,
      meta: {
        job_name: job.name,
  ...
  async uploadRows({ mappedData, job, serverName, filePath }) {
    ...
    for (let i = 0; i < chunks.length; i += 1) {
      ...
      const payload = this._buildSyncPayload({ wsConfig, job, mappedData: rowsChunk, filePath });
      payload.meta = {
        ...(payload.meta || {}),
        batch_id: batchId,
        chunk_index: i + 1,
      ...
      const result = await webServiceAuth.withAuthenticatedRequest(serverName, async (accessToken) => {
        try {
          return await this._postJson(syncUrl, payload, accessToken, wsConfig, `webservice sync (${serverName})`);
        } catch (error) {
  ...
  async testConnection(serverName) {
    ...
    if (wsConfig.testSyncOnConnect) {
      ...
      syncProbe = await webServiceAuth.withAuthenticatedRequest(serverName, async (accessToken) => {
        ...
        }, accessToken, wsConfig, `webservice sync probe (${serverName})`);
      });
    }
